...
id3v2AddLameVersion(lame_t gfp)
{
    ...
    if (lenb > 0) {
        sprintf_s(buffer, 1024, "LAME %s version %s (%s)", b, v, u);
    }
    else {
        sprintf_s(buffer, 1024, "LAME version %s (%s)", v, u);
    }
...
id3v2AddAudioDuration(lame_t gfp, double ms)
{
    ...
    sprintf_s(buffer, 1024, "%lu", playlength_ms);
...
lame_get_id3v1_tag(lame_t gfp, unsigned char *buffer, size_t size)
{
    ...
    if (test_tag_spec_flags(gfc, CHANGED_FLAG)) {
        ...
        *p++ = 'G';
		...
		//sprintf_s(year, 5, "%d", gfc->tag_spec.year);
